    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="520px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        <el-form-item label="角色标识" prop="roleKey">
          <el-input v-model="form.roleKey" style="width: 145px;" :disabled="crud.status.edit ===1" />
        </el-form-item>
      ...
      <el-col :xs="24" :sm="24" :md="16" :lg="16" :xl="17" style="margin-bottom: 10px">
        <el-card class="box-card" shadow="never">
          ...
          <el-table ref="table" v-loading="crud.loading" highlight-current-row style="width: 100%;" :data="crud.data" @selection-change="crud.selectionChangeHandler" @current-change="handleCurrentChange">
            ...
            <el-table-column v-if="columns.visible('roleKey')" prop="roleKey" label="角色标识" />
            <el-table-column v-if="columns.visible('name')" prop="name" label="角色名称" />
...
import { getEnableMenusTree } from '@/api/system/menu'
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '角色', url: 'role/list', sort: 'level,asc', crudMethod: { ...crudRoles }})
const defaultForm = { id: null, name: null, depts: [], remark: null, dataScope: '全部', level: 3, roleKey: null }
export default {
  name: 'Role',
  ...
  methods: {
    ...
    handleCurrentChange(val) {
      if (val) {
        ...
        this.menuIds = []
        // 菜单数据需要特殊处理
        val.menuIdList.forEach(function(id, index) {
    ...
    changeState(data, val) {
      ...
      this.$confirm('此操作将' + msg + '该角色' + ', 是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        const roleData = { id: data.id, status: 1 ^ data.status }
        crudRoles.changeState(roleData).then(res => {
    ...
    saveMenu() {
      ...
      const roleMenu = { roleId: this.currentId, menuIdList: [] }
      // 得到半选的父节点数据，保存起来
      this.$refs.menu.getHalfCheckedNodes().forEach(function(data, index) {
        roleMenu.menuIdList.push(data)
      })
      // 得到已选中的 key 值
      this.$refs.menu.getCheckedKeys().forEach(function(data, index) {
        roleMenu.menuIdList.push(data)
      })
      crudRoles.editMenu(roleMenu).then(res => {
    ...
    update() {
      // 无刷新更新 表格数据
      const role = { id: this.currentId }
      crudRoles.get(role).then(res => {
