  ...
  <div class="app-container">
    ...
    <el-dialog append-to-body :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" width="580px">
      <el-form ref="form" :inline="true" :model="form" :rules="rules" size="small" label-width="80px">
        ...
        <el-form-item v-show="form.type.toString() !== '2'" label="外链菜单" prop="isLinked">
          <el-radio-group v-model="form.isLinked" size="mini">
        ...
        <el-form-item v-show="form.type.toString() !== '2'" label="菜单可见" prop="visible">
        ...
        <el-form-item v-show="form.type.toString() !== '0'" label="权限标识" prop="permission">
          <el-input v-model="form.permission" :disabled="form.isLinked=== '0'" placeholder="权限标识" style="width: 178px;" />
        </el-form-item>
        ...
        <el-form-item v-show="form.isLinked.toString()=== '0' && form.type.toString() === '1'" label="组件名称" prop="componentName">
          <el-input v-model="form.componentName" style="width: 178px;" placeholder="匹配组件内Name字段" />
        </el-form-item>
        <el-form-item v-show="form.isLinked.toString()=== '0' && form.type.toString() === '1'" label="组件路径" prop="component">
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" :tree-props="{children: 'children', hasChildren: 'hasChildren'}" row-key="id" @select="crud.selectChange" @select-all="crud.selectAllChange" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-if="columns.visible('status')" label="状态" align="center" prop="status">
        <template slot-scope="scope">
          <span v-if="scope.row.status===1" style="color:#409EFF">启用中</span>
          <span v-else style="color:#F56C6C">禁用中</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="columns.visible('isLinked')" prop="isLinked" label="外链" width="75px">
        <template slot-scope="scope">
          <span v-if="scope.row.isLinked===1">是</span>
          <span v-else>否</span>
        </template>
      </el-table-column>
      ...
      <el-table-column v-if="columns.visible('visible')" prop="visible" label="菜单可见" width="75px">
        <template slot-scope="scope">
      ...
      <el-table-column label="操作" width="130px" align="center" fixed="right">
        <template slot-scope="scope">
          <el-switch
            ...
            v-permission="['admin','menu:status']"
            :active-value="1"
            :inactive-value="0"
            active-color="#409EFF"
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '菜单', url: '/menu/getMenuTree', crudMethod: { ...crudMenu }})
const defaultForm = { id: null, name: null, sort: 999, path: null, component: null, componentName: null, isLinked: 0, pid: 0, icon: null, cache: 0, visible: 0, type: 0, permission: null }
export default {
  name: 'Menu',
  ...
  methods: {
    ...
    changeState(data, val) {
      ...
      this.$confirm('此操作将' + msg + '该菜单' + ', 是否继续？', '提示', {
        ...
        const menuData = { id: data.id, status: 1 ^ data.status }
        crudMenu.changeState(menuData).then(res => {
