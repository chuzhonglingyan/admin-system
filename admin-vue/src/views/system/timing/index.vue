  ...
  <div class="app-container">
    ...
    <el-dialog :close-on-click-modal="false" :before-close="crud.cancelCU" :visible.sync="crud.status.cu > 0" :title="crud.status.title" append-to-body width="600px">
      <el-form ref="form" :model="form" :rules="rules" size="small" label-width="100px">
        <el-form-item label="任务组名称" prop="groupName">
          <el-input v-model="form.groupName" style="width: 460px;" :disabled="crud.status.edit ===1" />
        </el-form-item>
        ...
        <el-form-item label="任务状态">
          ...
          <el-radio v-model="form.status" :label="0">暂停</el-radio>
        </el-form-item>
    ...
    <el-table ref="table" v-loading="crud.loading" :data="crud.data" style="width: 100%;" @selection-change="crud.selectionChangeHandler">
      ...
      <el-table-column v-if="columns.visible('groupName')" :show-overflow-tooltip="true" prop="groupName" width="100px" label="任务组名称" />
      ...
      <el-table-column v-if="columns.visible('status')" :show-overflow-tooltip="true" prop="status" width="90px" label="状态">
        <template slot-scope="scope">
          <el-tag :type="scope.row.status ? 'warning' : 'success'">{{ scope.row.status ===0 ? '已暂停' : '运行中' }}</el-tag>
        </template>
      </el-table-column>
      ...
      <el-table-column v-permission="['admin','timing:edit','timing:del']" label="操作" width="180px" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button v-permission="['admin','timing:edit']" style="margin-left: 3px" type="text" size="mini" @click="changeState(scope.row,scope.row.status)">
...
// crud交由presenter持有
const defaultCrud = CRUD({ title: '定时任务', url: 'scheduleJob/list', crudMethod: { ...crudJob }})
const defaultForm = { id: null, groupName: null, jobName: null, beanName: null, methodName: null, params: null, cronExpression: null, status: 0, remark: null }
export default {
  name: 'Timing',
  ...
  methods: {
    ...
    changeState(data, status) {
      const msg = status === 0 ? '恢复' : '暂停'
      this.$confirm('此操作将' + msg + '该定时器' + ', 是否继续？', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
        const jobData = { id: data.id, status: status }
        crudJob.changeState(jobData).then(res => {
          this.crud.notify(msg + '成功', CRUD.NOTIFICATION_TYPE.SUCCESS)
          for (let i = 0; i < this.crud.data.length; i++) {
            if (jobData.id === this.crud.data[i].id) {
              const state = this.crud.data[i].status
              this.crud.data[i].status = ~state
              break
            }
          }
        }).catch(() => {
    ...
    updateStatus(id, status) {
      crudJob.changeState(id).then(res => {
        this.crud.toQuery()
